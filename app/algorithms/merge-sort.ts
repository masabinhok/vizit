...
// Helper to create a deep copy of array with metadata
const createArrayState = (
  values: number[],
  ...
  const { comparing = new Set(), merging = new Set(), sorted = new Set() } = options;
  return values.map((value, idx) => ({
    value,
    isComparing: comparing.has(originalIndices[idx]),
    isSwapping: merging.has(originalIndices[idx]), // Reusing isSwapping to indicate "being merged"
    isSorted: sorted.has(originalIndices[idx])
  }));
};
...
const recordStep = (
  ...
  const currentIndices = localIndices.map(idx => originalIndices[idx]);
  const comparing = new Set<number>();
  const merging = new Set<number>(currentIndices);
  
  steps.push({
    array: createArrayState(globalArray, originalIndices, { merging, comparing }),
    i: range?.start ?? -1,
    j: range?.end ?? -1,
    description,
    codeLineIndex,
...
const recordCompare = (
  ...
  const currentIndices = localIndices.map(idx => originalIndices[idx]);
  const comparing = new Set<number>([originalIndices[localIndices[leftIdx]], originalIndices[localIndices[rightIdx]]]);
  const merging = new Set<number>(currentIndices);
  
  steps.push({
    array: createArrayState(globalArray, originalIndices, { comparing, merging }),
    i: leftIdx,
    j: rightIdx,
    description,
    codeLineIndex,
...
const recordMergeResult = (
  ...
  mergedIndices.forEach((globalIdx, pos) => {
    globalArray[globalIdx] = mergedValues[pos];
  });

  const sorted = new Set<number>(mergedIndices);
  steps.push({
    array: createArrayState(globalArray, originalIndices, { sorted }),
    i: mergedIndices[0],
    j: mergedIndices[mergedIndices.length - 1],
    description,
    codeLineIndex,
    comparisons,
    swaps: (swaps += mergedValues.length)
  });
};
...
const mergeSortRecursive = (
  ...
  localIndices: number[],
  depth: number = 0
): { sortedValues: number[]; sortedIndices: number[] } => {
  const n = values.length;
  
  // Base case
  if (n <= 1) {
    if (n === 1) {
      recordStep(values, localIndices, `Base case reached for [${values[0]}]`, 1);
    }
    return { sortedValues: [...values], sortedIndices: [...localIndices] };
  }
  ...
  const leftIndices = localIndices.slice(0, mid);
  const rightIndices = localIndices.slice(mid);

  recordStep(
    values,
    localIndices,
    `Dividing [${values.join(', ')}] into [${leftValues.join(', ')}] and [${rightValues.join(', ')}]`,
    2,
    { start: localIndices[0], end: localIndices[n - 1] }
  );

  // Recursively sort halves
  const leftResult = mergeSortRecursive(leftValues, leftIndices, depth + 1);
  const rightResult = mergeSortRecursive(rightValues, rightIndices, depth + 1);

  // Merge
  recordStep(
    [...leftResult.sortedValues, ...rightResult.sortedValues],
    [...leftResult.sortedIndices, ...rightResult.sortedIndices],
    `Merging [${leftResult.sortedValues.join(', ')}] and [${rightResult.sortedValues.join(', ')}]`,
    9
  );

  // Perform merge with comparisons
  const mergedValues: number[] = [];
  const mergedIndices: number[] = [];
  let i = 0, j = 0;

  while (i < leftResult.sortedValues.length && j < rightResult.sortedValues.length) {
    recordCompare(
      [...leftResult.sortedValues, ...rightResult.sortedValues],
      [...leftResult.sortedIndices, ...rightResult.sortedIndices],
      i,
      leftResult.sortedValues.length + j,
      `Comparing ${leftResult.sortedValues[i]} and ${rightResult.sortedValues[j]}`,
      11
    );

    if (leftResult.sortedValues[i] <= rightResult.sortedValues[j]) {
      mergedValues.push(leftResult.sortedValues[i]);
      mergedIndices.push(leftResult.sortedIndices[i]);
      i++;
    } else {
      mergedValues.push(rightResult.sortedValues[j]);
      mergedIndices.push(rightResult.sortedIndices[j]);
      j++;
    }
  }

  // Add remaining elements
  while (i < leftResult.sortedValues.length) {
    mergedValues.push(leftResult.sortedValues[i]);
    mergedIndices.push(leftResult.sortedIndices[i]);
    i++;
  }
  while (j < rightResult.sortedValues.length) {
    mergedValues.push(rightResult.sortedValues[j]);
    mergedIndices.push(rightResult.sortedIndices[j]);
    j++;
  }
  ...
  recordMergeResult(
    mergedValues,
    mergedIndices,
    `Merged into [${mergedValues.join(', ')}]`,
    18
  );

  return { sortedValues: mergedValues, sortedIndices: mergedIndices };
};
