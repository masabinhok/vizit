 ...
 * It yields each step for visualization.
 */
function* lisPatienceSteps(arr: number[]) {
  const piles: number[][] = [];
  yield { piles: [...piles.map(p => [...p])], message: "Start with no piles" };

  for (const x of arr) {
    // Binary search to find leftmost pile whose top >= x
  ...
  yield { piles: piles.map(p => [...p]), message: `Final piles formed (LIS length = ${piles.length})` };
}
        ...
        {current.piles.map((pile, i) => (
          <div key={i} className="flex flex-col-reverse items-center">
            {pile.map((v, j) => (
              <div
                key={j}
                className="w-10 h-10 bg-emerald-600 flex items-center justify-center rounded-md text-white font-semibold mb-1 shadow-md"
              >
                {v}
              </div>
            ))}
