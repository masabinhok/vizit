...
const Toast = ({ message, onClose, isDarkMode }: { message: string, onClose: () => void, isDarkMode: boolean }) => (
  <div className={`fixed top-24 right-5 z-50 p-4 rounded-lg shadow-lg flex items-center ${isDarkMode ? 'bg-red-700 text-white' : 'bg-red-600 text-white'} animate-in fade-in slide-in-from-top-5`}>
...
export default function RadixSortPage() {
  ...
  const [steps, setSteps] = useState<AlgorithmStep[]>([]); // This will be RadixSortStep[]
  ...
  // All hooks below are copied and will work as-is
  useEffect(() => {
    if (alertMessage) {
  ...
  const initializeAlgorithm = () => {
    try {
      ...
      if (numbers.length === 0) {
        setAlertMessage("Please enter valid numbers separated by commas");
        return;
      }
      ...
      // Check for negative numbers (our LSD algorithm handles positive only)
      if (numbers.some(n => n < 0)) {
        setAlertMessage("This Radix Sort visualization only supports positive numbers.");
        return;
      }
      ...
      setAlertMessage("Invalid input format. Please use numbers separated by commas.");
    }
  };
  ...
  const generateRandomArray = () => {
    ...
    const newArrayString = arr.join(',');
    setInputArray(newArrayString);
    
    
    try {
      const newSteps = algorithmConfig.generateSteps(arr);
      setSteps(newSteps);
      setCurrentStep(0);
      setIsInitialized(true);
      setIsPlaying(false);
      setAlertMessage("Generated new random array!");
    } catch (error) {
      console.error("Algorithm initialization error:", error);
      setAlertMessage("Error generating random array.");
    }
  };
  ...
  // The JSX is identical, just consuming the new `algorithmConfig`
  return (
    <div className={`flex flex-col h-screen ${isDarkMode ? 'bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900' : 'bg-gradient-to-br from-slate-50 via-blue-50/30 to-purple-50/30'} transition-all duration-500 relative overflow-hidden`}>
      {alertMessage && <Toast message={alertMessage} onClose={() => setAlertMessage(null)} isDarkMode={isDarkMode} />}
